...
export function gradeFromDeltaPct(x:number):Grade{
  if(x>=0.25)return'A+'; if(x>=0.18)return'A'; if(x>=0.12)return'A-';
  if(x>=0.08)return'B+'; if(x>=0.04)return'B'; if(x>=0.00)return'B-';
  if(x>=-0.04)return'C+'; if(x>=-0.08)return'C'; if(x>=-0.12)return'C-';
  if(x>=-0.20)return'D'; return'F';
}

/** Return a readable hex color for the grade */
